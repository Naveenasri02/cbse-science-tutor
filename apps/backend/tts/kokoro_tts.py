...
class KokoroTTS:
    ...
    Device controlled by TTS_DEVICE env var ('cpu' default for memory-tight pods
    sharing GPU with vLLM; 'cuda' for dedicated GPU). CPU is fast enough for
    Kokoro (~100ms per sentence) and avoids OOM on shared GPU."""

    def __init__(self):
        model_path = config.TTS_MODEL_PATH
        voices_path = config.TTS_VOICES_PATH
        device = os.getenv("TTS_DEVICE", "cpu").lower()
        print(f"  Loading Kokoro TTS ({device.upper()})...")
