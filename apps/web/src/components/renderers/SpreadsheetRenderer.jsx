...
import { normalize, clearHighlights, highlightWithRetry, ViewerToolbar } from './highlightUtils'

export default function SpreadsheetRenderer({ fileUrl, filename, targetSnippet, targetFallbackSnippet, targetRequestId, onClose }) {
  ...
  // DOM-based highlighting with retry (mirrors PdfRenderer approach)
  useEffect(() => {
    if (!targetSnippet || !targetRequestId || !tableRef.current) return
    clearHighlights(tableRef.current)
    const cleanup = highlightWithRetry(tableRef.current, targetSnippet, targetFallbackSnippet, targetRequestId, genRef)
    return cleanup
  }, [targetSnippet, targetFallbackSnippet, targetRequestId, sheets, activeSheet])
